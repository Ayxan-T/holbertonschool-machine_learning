...
def autoencoder(input_dims, hidden_layers, latent_dims, lambtha):
    ...
    encoder = keras.models.Model(input=encoder_input, output=encoder_output)
    ...
    for num_neurons in reversed(hidden_layers[:-1]):
        x = keras.layers.Dense(num_neurons, activation="relu")(x)
    
    decoder_output = keras.layers.Dense(input_dims, activation="sigmoid")(x)

    decoder = keras.models.Model(input=decoder_input, output=decoder_output)
    ...
    auto = keras.models.Model(input=auto_input, output=auto_output)
